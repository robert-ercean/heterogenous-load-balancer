...
//go:generate bpf2go -cc clang -cflags "-O2 -g -Wall -I../bpf/include" -target amd64 xdpreturn   ../bpf/xdp_return.c

type Loader struct {
	xdpForwardObjs xdpforwardObjects
	xdpReturnObjs  xdpreturnObjects

	xdpForwardLink link.Link
	xdpReturnLink  link.Link
...
func New() (*Loader, error) {
	// Remove memory limit on locked memory 
	...
	if err := loadXdpforwardObjects(&l.xdpForwardObjs, nil); err != nil {
...
func (l *Loader) AttachXDPForward(ifaceName string) error {
	...
	link, err := link.AttachXDP(link.XDPOptions{
		Program:   l.xdpForwardObjs.XdpForward,
		Interface: iface.Index,
		// Flags:     link.XDPGenericMode,
	})
	if err != nil {
		return fmt.Errorf("[BPF_LOADER] attach XDP Forward to %s: %w", ifaceName, err)
	}

	l.xdpForwardLink = link
	log.Printf("[BPF_LOADER] XDP Forwardattached to %s", ifaceName)
	return nil
}

func (l *Loader) AttachXDPReturn(ifaceName string) error {
	iface, err := net.InterfaceByName(ifaceName)
	if err != nil {
		return fmt.Errorf("[BPF_LOADER] interface %s: %w", ifaceName, err)
	}
	...
	link, err := link.AttachXDP(link.XDPOptions{
		Program:   l.xdpReturnObjs.XdpReturn,
		Interface: iface.Index,
		// Flags:     link.XDPGenericMode,
	})
	if err != nil {
		return fmt.Errorf("[BPF_LOADER] attach XDP Return to %s: %w", ifaceName, err)
	}

	l.xdpReturnLink = link
	log.Printf("[BPF_LOADER] XDP Return attached to %s", ifaceName)
	return nil
}
...
func (l *Loader) SetVIP(vip net.IP) error {
	...
	key := uint32(0)
	if err := l.xdpForwardObjs.VipMap.Update(key, vipU32, ebpf.UpdateAny); err != nil {
...
func (l *Loader) SetLBEgressIP(ip net.IP) error {
	...
	key := uint32(0)
	if err := l.xdpReturnObjs.LbBridgeIp.Update(key, val, ebpf.UpdateAny); err != nil {
...
func (l *Loader) SetVIPTCPPort(port uint16) error {
	val := uint32(port)
	key := uint32(0)
	if err := l.xdpReturnObjs.VipTcpPort.Update(key, val, ebpf.UpdateAny); err != nil {
...
func (l *Loader) SetBackend(pool Pool, ip net.IP, port uint16, loadScore uint32, mac net.HardwareAddr) error {
	...
	switch pool {
	case PoolTCP:
		poolMap = l.xdpForwardObjs.TcpPool
		metaKey = 0
		poolName = "tcp"
	case PoolUDP:
		poolMap = l.xdpForwardObjs.UdpPool
		metaKey = 1
		poolName = "udp"
	default:
	...
	if err := l.xdpForwardObjs.PoolMeta.Lookup(metaKey, &currentCount); err != nil {
	...
	if err := l.xdpForwardObjs.PoolMeta.Update(metaKey, newCount, ebpf.UpdateAny); err != nil {
...
func (l *Loader) Close() error {
	if l.xdpForwardLink != nil {
		l.xdpForwardLink.Close()
	}
	if l.xdpReturnLink != nil {
		l.xdpReturnLink.Close()
	}
	l.xdpForwardObjs.Close()
	l.xdpReturnObjs.Close()
	return nil
}
