...
const (
	...
	// HeartbeatIntervalSec is the interval at which backends must heartbeat.
	// Imported by both the control plane and the backend agent so both sides
	// agree without negotiating it on the wire.
	HeartbeatIntervalSec uint8 = 5
)
